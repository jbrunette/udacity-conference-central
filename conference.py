...
from models import SessionsByTypeQueryForm
from models import SessionsBySpeakerQueryForm
...
# - - - Sessions - - - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        ...
        """Return sessions for provided conference (by websafeConferenceKey)."""
        # get Conference object from request; bail if not found
        conf = ndb.Key(urlsafe=request.websafeConferenceKey).get()
        if not conf:
            raise endpoints.NotFoundException(
        ...
        # Query for sessions for conference
        q = Session.query()
        q = q.filter(Session.conferenceName == conf.name)

        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in q]
        )

    @endpoints.method(SessionsByTypeQueryForm, SessionForms,
            path='sessionsByType',
            http_method='POST', name='getConferenceSessionsByType')
    def getConferenceSessionsByType(self, request):

        """Return sessions for provided conference (by websafeConferenceKey and type)."""
        # get Conference object from request; bail if not found
        conf = ndb.Key(urlsafe=request.websafeConferenceKey).get()
        if not conf:
            raise endpoints.NotFoundException(
        ...
        # Query for sessions for conference and type
        q = Session.query()
        q = q.filter(Session.websafeConferenceKey == request.websafeConferenceKey)
        q = q.filter(Session.typeOfSession == request.type)

        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in q]
        )

    @endpoints.method(SessionsBySpeakerQueryForm, SessionForms,
            path='sessionsBySpeaker',
            http_method='POST', name='getConferenceSessionsBySpeaker')
    def getConferenceSessionsBySpeaker(self, request):

        """Return sessions for provided conference (by websafeConferenceKey and speaker)."""
    ...
    def createSession(self, request):
        ...
        speakerSessions = Session.query(Session.speaker==data['speaker'], Session.websafeConferenceKey==request.websafeConferenceKey)
        if speakerSessions.count() > 1:
            taskqueue.add(params={'speaker': data['speaker'],
                'sessions':', '.join([s.name for s in speakerSessions])},
    ...
    def getSessionsInWishlist(self, request):
        """Get list of sessions user has in their wishlist."""
        prof = self._getProfileFromUser() # get user Profile
...
# - - - Task 3  - - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(SESS_GET_REQUEST, ProfileForms,
            path='getSessionWishlistUsers/{websafeSessionKey}',
            http_method='GET', name='getSessionWishlistUsers')
    def getSessionWishlistUsers(self, request):
        """Returns user profiles for those who have the provided session in their wishlist."""
        q = Profile.query(Profile.sessionWishlist.IN([request.websafeSessionKey]))

        # return set of ConferenceForm objects per Conference
        return ProfileForms(items=[self._copyProfileToForm(prof) for prof in q])
    
    @endpoints.method(CONF_GET_REQUEST, ProfileForms,
            path='getUsersAttendingConference/{websafeConferenceKey}',
            http_method='GET', name='getUsersAttendingConference')
    def getUsersAttendingConference(self, request):
        """Returns user profiles for those who are attending provided conference."""
        q = Profile.query(Profile.conferenceKeysToAttend.IN([request.websafeConferenceKey]))
...
# - - - Task 4  - - - - - - - - - - - - - - - - - - - -
    ...
    def getFeaturedSpeaker(self, request):
        """Returns name of speaker who is the featured speaker from memCache."""
        # Return Speaker from memcache
        return StringMessage(data=memcache.get('speaker') or "")
